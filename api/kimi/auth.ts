...
// Lazy JWKS — only created when kimiAuthUrl is configured
let _jwks: ReturnType<typeof jose.createRemoteJWKSet> | null = null;
function getJwks() {
  if (!env.kimiAuthUrl) throw new Error("KIMI_AUTH_URL not configured");
  if (!_jwks) _jwks = jose.createRemoteJWKSet(new URL(`${env.kimiAuthUrl}/api/.well-known/jwks.json`));
  return _jwks;
}
