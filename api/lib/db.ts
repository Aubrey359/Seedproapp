...
export async function seedCropGuides(): Promise<void> {
  ...
  const guideData = [
    ...
    // ── Onion (cropId 2) — day 0 = transplanting nursery seedlings ──
    { cropId: 2, stage: "Establishment", stageOrder: 1, dayFrom: 0, dayTo: 14, title: "Establishment (Day 0-14)",
      description: "Transplanted seedlings settle in and begin rooting.",
      tasks: ["Transplant healthy 6-8 week old nursery seedlings, spacing about 20cm between rows and 10cm within rows", "Water immediately after transplanting to reduce shock", "Apply basal DAP fertilizer at or just before transplanting", "Gap-fill any seedlings that die within the first week"],
      tips: ["Transplant in the late afternoon or on a cloudy day to reduce transplant shock"],
      warnings: ["Cutworms can cut young seedlings at the base right after transplanting", "Damping-off can carry over from the nursery in poorly drained soil"] },
    { cropId: 2, stage: "Vegetative Growth", stageOrder: 2, dayFrom: 14, dayTo: 45, title: "Vegetative Growth (Day 14-45)",
      description: "The plant builds up leaves — more leaf growth now generally means a bigger bulb later.",
      tasks: ["First weeding around 2-3 weeks after transplanting — onions compete poorly with weeds", "Top-dress with CAN around 4-5 weeks", "Keep soil consistently moist but not waterlogged", "Start scouting for thrips on the leaves"],
      tips: [],
      warnings: ["Thrips cause silvery streaking on leaves and can spread viruses — check the leaf axils regularly", "Purple blotch can appear on leaves in humid conditions"] },
    { cropId: 2, stage: "Bulb Initiation", stageOrder: 3, dayFrom: 45, dayTo: 75, title: "Bulb Initiation (Day 45-75)",
      description: "The plant shifts from leaf growth to forming a bulb at the base.",
      tasks: ["Reduce nitrogen after this point — too much late nitrogen delays bulbing and encourages thick necks", "Keep weeding — competition now directly reduces bulb size", "Maintain even soil moisture"],
      tips: ["Bulbing is triggered mainly by day length and variety — check your seed source for one suited to your area"],
      warnings: ["Downy mildew thrives in cool, humid conditions and can spread fast through a field", "Onion thrips pressure often peaks around now"] },
    { cropId: 2, stage: "Bulb Bulking", stageOrder: 4, dayFrom: 75, dayTo: 100, title: "Bulb Bulking (Day 75-100)",
      description: "Bulbs swell and put on most of their final size.",
      tasks: ["Begin easing off irrigation as bulbs near full size", "Avoid mechanical damage to bulbs while weeding", "Keep monitoring for pests and disease right up to maturity"],
      tips: [],
      warnings: ["Neck rot and bacterial soft rot risk rises if the crop stays wet close to maturity", "Continue watching for thrips and downy mildew"] },
    { cropId: 2, stage: "Maturity & Harvest", stageOrder: 5, dayFrom: 100, dayTo: undefined, title: "Maturity & Harvest (Day 100-130)",
      description: "Tops yellow and fall over (\"neck fall\") as the bulb finishes maturing.",
      tasks: ["Stop irrigation about 2-3 weeks before harvest", "Harvest once 50-80% of tops have naturally fallen over", "Cure bulbs in a well-ventilated, shaded place for 1-2 weeks before storage or sale — this toughens the neck and skin"],
      tips: ["Well-cured bulbs store far longer and fetch better prices than freshly pulled ones"],
      warnings: ["Bulbs left in wet soil past maturity are prone to rot", "Handle harvested bulbs gently — bruised bulbs rot faster in storage"] },

    // ── Coffee (cropId 4) — day 0 = field planting of a nursery seedling ──
    { cropId: 4, stage: "Field Establishment", stageOrder: 1, dayFrom: 0, dayTo: 90, title: "Field Establishment (Day 0-90)",
      description: "A young coffee seedling is transplanted from the nursery into its permanent field position and begins establishing its root system.",
      tasks: ["Plant during the onset of reliable rains, in holes prepared with topsoil and well-rotted manure", "Provide temporary shade in hot, exposed sites for the first few months", "Water during dry spells until the seedling is well established", "Weed regularly around the base — young coffee competes poorly with weeds"],
      tips: ["Standard spacing is roughly 2-2.7m between trees depending on variety and soil fertility — wider spacing on richer soils"],
      warnings: ["Termites and cutworms can kill young seedlings outright", "Waterlogging kills coffee seedlings quickly — avoid poorly drained sites"] },
    { cropId: 4, stage: "Formative Growth & Training", stageOrder: 2, dayFrom: 90, dayTo: 365, title: "Formative Growth & Training (Day 90-365)",
      description: "The young tree builds its permanent framework of branches through its first year.",
      tasks: ["Remove any suckers below the first fork to establish a clean stem", "Apply manure or compost at the start of the rains", "Continue weeding and mulching to conserve moisture", "Begin scouting for leaf rust and early pest pressure"],
      tips: [],
      warnings: ["Leaf rust and other fungal issues can set back a young tree significantly if untreated", "Coffee wilt disease is untreatable once established — remove and destroy any wilting tree to protect the rest of the plot"] },
    { cropId: 4, stage: "Second Year Development", stageOrder: 3, dayFrom: 365, dayTo: 730, title: "Second Year Development (Day 365-730)",
      description: "The tree continues building height and branch structure, usually flowering lightly for the first time this year.",
      tasks: ["Apply split fertilizer applications timed with the rains", "Continue formative pruning to build a strong, open framework", "Keep up pest and disease scouting, especially for coffee berry borer once any berries form"],
      tips: ["A light first flowering this year is normal — don't expect a full harvest yet"],
      warnings: ["Coffee berry disease and berry borer can affect even the first small crop of berries"] },
    { cropId: 4, stage: "First Full Bearing", stageOrder: 4, dayFrom: 730, dayTo: 1095, title: "First Full Bearing (Day 730-1095)",
      description: "The tree reaches its first substantial harvest, typically in its third year in the field.",
      tasks: ["Apply the full mature-tree fertilizer program, split across two applications timed with the rains", "Harvest selectively, picking only fully red ripe cherries in rounds", "Begin a regular pruning routine after this first main harvest to set up future years"],
      tips: [],
      warnings: ["Keep controlling coffee berry borer and coffee berry disease as bearing increases — losses hurt more once there's a real crop to protect"] },
    { cropId: 4, stage: "Mature Bush", stageOrder: 5, dayFrom: 1095, dayTo: undefined, title: "Mature Bush — Ongoing Annual Cycle (Day 1095+)",
      description: "From here the tree settles into a repeating annual cycle: post-harvest pruning, flowering after the rains, berry development, and selective harvest.",
      tasks: ["Prune after each harvest to remove old and diseased wood and keep the bush productive", "Apply fertilizer in two splits timed with the rains each year", "Strip-pick any leftover berries after each harvest to control coffee berry borer", "Harvest selectively in rounds as cherries ripen"],
      tips: [],
      warnings: ["A mature bush left unpruned or unfed for a season will show a clear drop in the following year's yield"] },

    // ── Tea (cropId 9) — day 0 = field planting of a rooted cutting ──
    { cropId: 9, stage: "Field Establishment", stageOrder: 1, dayFrom: 0, dayTo: 90, title: "Field Establishment (Day 0-90)",
      description: "A rooted tea cutting is transplanted into the field and begins establishing itself.",
      tasks: ["Plant at the onset of reliable rains, at the spacing recommended for your area (commonly around 1.2m between rows and 0.6-0.75m within rows)", "Mulch around the base to conserve moisture and suppress weeds", "Water during dry spells until established", "Provide temporary shade in hot, exposed sites if plants look stressed"],
      tips: [],
      warnings: ["Young tea plants are very sensitive to drought in their first few months", "Weeds compete heavily with young tea — keep the base clear"] },
    { cropId: 9, stage: "Formative Pruning & Bush Building", stageOrder: 2, dayFrom: 90, dayTo: 730, title: "Formative Pruning & Bush Building (Day 90-730)",
      description: "Over the first one to two years, the young plant is repeatedly pruned low to force it to bush out sideways instead of growing as a single tall stem — this shapes the plucking table for the plant's entire productive life.",
      tasks: ["Follow a formative pruning/pegging routine to spread the bush wide and low rather than letting it grow tall", "Apply fertilizer at the start of each rainy season to support vigorous vegetative growth", "Keep weeding and mulching around the base", "Do not pluck yet — all growth goes into building the bush frame"],
      tips: ["A well-shaped young bush pays off for decades — rushing formative pruning to get an early pluck reduces yield for the life of the bush"],
      warnings: ["Poor formative pruning now leads to a narrow, low-yielding plucking table for the rest of the bush's life"] },
    { cropId: 9, stage: "First Plucking", stageOrder: 3, dayFrom: 730, dayTo: 1095, title: "First Plucking (Day 730-1095)",
      description: "The bush is brought into light plucking for the first time, usually around its third year.",
      tasks: ["Begin light, careful plucking of \"two leaves and a bud\" once the plucking table is well established", "Continue regular fertilizer applications timed with the rains", "Watch closely for red spider mite and mirids (tea mosquito bug), which target new, tender growth"],
      tips: [],
      warnings: ["Over-plucking a young bush before it's ready sets back long-term yield — go easy in this first plucking year"] },
    { cropId: 9, stage: "Mature Bush", stageOrder: 4, dayFrom: 1095, dayTo: undefined, title: "Mature Bush — Continuous Plucking (Day 1095+)",
      description: "From here the bush is plucked on a continuous rotation, with a major pruning every few years to keep it productive.",
      tasks: ["Pluck \"two leaves and a bud\" on a round of roughly 7-14 days depending on growth rate and season", "Apply fertilizer in split applications timed with the rains, typically 2-3 times a year", "Prune the bush back every 3-4 years to control height and rejuvenate growth", "Keep monitoring for red spider mite, mirids/tea mosquito bug, and thrips"],
      tips: ["Plucking round length should track how fast new shoots grow — shorter rounds in wet, fast-growth periods, longer rounds when growth slows"],
      warnings: ["Skipping plucking rounds lets shoots grow coarse and reduces both quality and the price you'll fetch", "A bush that's overdue for pruning becomes patchy and harder to pluck evenly"] },
  ];
  ...
  const scheduleData = [
    ...
    // Onion
    { cropId: 2, stage: "Establishment",     dayFrom: 0,   dayTo: 0,   activityType: "fertilizer", productName: "DAP (basal)",           dosage: "~75 kg/acre typical", ratePerAcreKg: 75, instructions: "Apply at or just before transplanting." },
    { cropId: 2, stage: "Vegetative Growth", dayFrom: 28,  dayTo: 35,  activityType: "fertilizer", productName: "CAN (top-dress)",       dosage: "~50 kg/acre",         ratePerAcreKg: 50, instructions: "Apply once around 4-5 weeks after transplanting — avoid heavy nitrogen after bulbing starts." },
    { cropId: 2, stage: "Bulb Bulking",      dayFrom: 90,  dayTo: 100, activityType: "irrigation", productName: undefined,               dosage: undefined,             instructions: "Begin reducing irrigation as bulbs approach full size to help skins toughen for storage." },
    { cropId: 2, stage: "Maturity & Harvest", dayFrom: 110, dayTo: 130, activityType: "harvest",   productName: undefined,               dosage: undefined,             instructions: "Harvest once most tops have fallen over, then cure in a shaded, ventilated spot for 1-2 weeks before storage." },
    // Coffee — per-tree/per-acre rates vary too much with spacing and tree age to state a single safe ratePerAcreKg; dosage stays descriptive only.
    { cropId: 4, stage: "Field Establishment",          dayFrom: 0,   dayTo: 14,   activityType: "fertilizer", productName: "Basal manure (planting hole)",              dosage: undefined, instructions: "Mix well-rotted manure or compost into the topsoil used to fill the planting hole." },
    { cropId: 4, stage: "Formative Growth & Training",  dayFrom: 90,  dayTo: 120,  activityType: "fertilizer", productName: "NPK compound fertilizer (young tree)",      dosage: "Rates vary by tree age and local soil — check with your local coffee extension officer or cooperative for current recommendations", instructions: "Apply at the start of the rains alongside manure or compost." },
    { cropId: 4, stage: "Second Year Development",      dayFrom: 400, dayTo: 430,  activityType: "pruning",    productName: undefined,                                   dosage: undefined, instructions: "Continue formative pruning to build a strong, open branch framework ahead of first bearing." },
    { cropId: 4, stage: "First Full Bearing",           dayFrom: 730, dayTo: 760,  activityType: "pesticide",  productName: "Coffee berry borer control",                dosage: undefined, instructions: "Begin regular monitoring and control of coffee berry borer as the first substantial crop of berries develops." },
    { cropId: 4, stage: "Mature Bush",                  dayFrom: 1095, dayTo: 1125, activityType: "fertilizer", productName: "NPK compound fertilizer (mature bush, 1st split)", dosage: "Typically split into two applications a year timed with the rains — get exact per-tree rates from your local coffee extension officer or cooperative", instructions: "Apply the first split just before or at the start of flowering." },
    // Tea — same reasoning as coffee: no safe single per-acre rate, dosage stays descriptive only.
    { cropId: 9, stage: "Field Establishment",              dayFrom: 0,    dayTo: 14,   activityType: "fertilizer", productName: "Basal manure/compost",                          dosage: undefined, instructions: "Work well-rotted manure or compost into the planting hole before transplanting the rooted cutting." },
    { cropId: 9, stage: "Formative Pruning & Bush Building", dayFrom: 90,   dayTo: 120,  activityType: "fertilizer", productName: "NPK fertilizer (formative growth)",             dosage: "Rates vary significantly by soil and cultivar — check with your local tea extension officer for the current recommendation", instructions: "Apply at the start of the rains to support vigorous formative growth; do not pluck yet." },
    { cropId: 9, stage: "First Plucking",                    dayFrom: 730,  dayTo: 760,  activityType: "pesticide",  productName: "Red spider mite / mirid (tea mosquito bug) control", dosage: undefined, instructions: "Start close monitoring for red spider mite and mirids as the bush comes into its first light plucking — both target tender new growth." },
    { cropId: 9, stage: "Mature Bush",                       dayFrom: 1095, dayTo: 1125, activityType: "fertilizer", productName: "NPK fertilizer (mature bush)",                  dosage: "Typically split into 2-3 applications a year timed with the rains — get exact rates from your local tea extension officer, as they vary by soil and altitude", instructions: "Apply as the rains begin to support new flush growth." },
  ];
